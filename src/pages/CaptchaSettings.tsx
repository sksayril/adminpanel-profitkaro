...
import { Shield, Save, RefreshCw, Coins, Wallet, AlertCircle, CheckCircle2 } from 'lucide-react';
import Sidebar from '../components/Sidebar';
import Header from '../components/Header';
import { getCaptchaSettings, setCaptchaSettings, CaptchaSettingsRequest } from '../services/api';
...
const CaptchaSettings = () => {
  ...
  const [formData, setFormData] = useState<CaptchaSettingsRequest>({
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      if (response.data) {
        setFormData({
          DailyCaptchaLimit: response.data.DailyCaptchaLimit,
          RewardPerCaptcha: response.data.RewardPerCaptcha,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (formData.DailyCaptchaLimit <= 0) {
      setError('Daily Captcha Limit must be greater than 0');
      setLoading(false);
      return;
    }

    if (formData.RewardPerCaptcha < 0) {
      setError('Reward Per Captcha must be 0 or greater');
      setLoading(false);
      return;
    }

    try {
      const response = await setCaptchaSettings(formData);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData({
      ...formData,
      [name]: name === 'DailyCaptchaLimit' || name === 'RewardPerCaptcha' ? Number(value) : value,
    });
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">Captcha Settings</h1>
                <p className="text-gray-500 text-sm">Configure captcha limits and rewards</p>
              </div>
            </div>
          </div>
              ...
              <form onSubmit={handleSubmit} className="p-8">
                ...
                <div className="mb-8">
                  ...
                  <div className="relative">
                    <input
                      type="number"
                      name="DailyCaptchaLimit"
                      value={formData.DailyCaptchaLimit}
                      onChange={handleChange}
                      min="1"
                      required
                      className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-200 transition-all text-gray-800"
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    Maximum number of captchas a user can solve per day (must be greater than 0)
                  </p>
                </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="relative">
                    <input
                      type="number"
                      name="RewardPerCaptcha"
                      value={formData.RewardPerCaptcha}
                      onChange={handleChange}
                      min="0"
                      step="0.01"
                      required
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    Reward amount per captcha solve (must be 0 or greater)
                  </p>
                </div>
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-900 mb-1">About Captcha Settings</h3>
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• Daily limit applies per user and resets at midnight</li>
                  <li>• Reward amount can be set to 0 to disable rewards</li>
                  <li>• Changes take effect immediately after saving</li>
                  <li>• Default values: 10 daily limit, 1 reward, Coins type</li>
                </ul>
              </div>
            </div>
          </div>
