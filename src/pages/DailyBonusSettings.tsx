...
import { Calendar, Save, RefreshCw, Coins, Wallet, AlertCircle, CheckCircle2, TrendingUp } from 'lucide-react';
import Sidebar from '../components/Sidebar';
import Header from '../components/Header';
import { getDailyBonusSettings, setDailyBonusSettings, DailyBonusSettingsRequest } from '../services/api';
...
const DailyBonusSettings = () => {
  ...
  const [formData, setFormData] = useState<DailyBonusSettingsRequest>({
    Monday: 0,
    Tuesday: 0,
    Wednesday: 0,
    Thursday: 0,
  ...
  const fetchSettings = async () => {
    ...
    setError('');
    try {
      const response = await getDailyBonusSettings();
      if (response.data) {
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidDays = daysOfWeek.filter(
      (day) => formData[day.key as keyof DailyBonusSettingsRequest] < 0
    );

    if (invalidDays.length > 0) {
      setError('All bonus amounts must be 0 or greater');
      setLoading(false);
      return;
    }

    try {
      const response = await setDailyBonusSettings(formData);
  ...
  // Calculate total weekly bonus
  const totalWeeklyBonus = daysOfWeek.reduce(
    (sum, day) => sum + (formData[day.key as keyof DailyBonusSettingsRequest] as number),
    0
  );
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">Daily Bonus Settings</h1>
                <p className="text-gray-500 text-sm">Configure daily bonus amounts for each day of the week</p>
              </div>
            </div>
          </div>
              ...
              <form onSubmit={handleSubmit} className="p-8">
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {daysOfWeek.map((day) => {
                      const value = formData[day.key as keyof DailyBonusSettingsRequest] as number;
                            ...
                            <input
                              ...
                              className={`w-full px-4 py-3 rounded-lg border-2 focus:outline-none focus:ring-2 transition-all text-gray-800 font-medium ${
                                isWeekend
                                  ...
                                  : 'border-gray-300 focus:border-orange-500 focus:ring-orange-200 bg-white'
                              }`}
                              placeholder="0"
                            />
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-orange-700">
                          {totalWeeklyBonus.toLocaleString()} {formData.RewardType === 'Coins' ? 'Coins' : '₹'}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold text-orange-600">
                        {(totalWeeklyBonus / 7).toFixed(2)} {formData.RewardType === 'Coins' ? 'Coins' : '₹'}
                      </p>
                    </div>
                  </div>
                </div>
                        ...
                        <div className="text-left">
                          <div
                            className={`font-semibold ${
                              formData.RewardType === 'Coins' ? 'text-orange-700' : 'text-gray-700'
                            }`}
                          >
                            Coins
                          </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <div
                            className={`font-semibold ${
                              formData.RewardType === 'WalletBalance' ? 'text-orange-700' : 'text-gray-700'
                            }`}
                          >
                            Wallet Balance
                          </div>
                ...
                <div className="flex gap-4 pt-6 border-t border-gray-200">
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-gradient-to-r from-orange-600 to-amber-600 text-white py-3 px-6 rounded-xl font-semibold hover:from-orange-700 hover:to-amber-700 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {loading ? (
                      <>
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-900 mb-1">About Daily Bonus Settings</h3>
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• Users can claim each day's bonus once per week</li>
                  <li>• Weekly reset happens automatically</li>
                  <li>• All bonus amounts must be 0 or greater</li>
                  <li>• Weekend bonuses (Saturday & Sunday) are highlighted</li>
          ...
          <div className="mt-6 bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
              ...
              Weekly Bonus Overview
            </h3>
            <div className="space-y-3">
              {daysOfWeek.map((day) => {
                const value = formData[day.key as keyof DailyBonusSettingsRequest] as number;
                const percentage = totalWeeklyBonus > 0 ? (value / totalWeeklyBonus) * 100 : 0;
                const isWeekend = day.key === 'Saturday' || day.key === 'Sunday';
                return (
...
export default DailyBonusSettings;
